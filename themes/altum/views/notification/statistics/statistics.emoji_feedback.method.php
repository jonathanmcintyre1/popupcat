...
<div class="table-responsive table-custom-container">
    <table class="table table-custom">
        ...
        <?php while($row = $result->fetch_object()): ?>
            <tr>
                <td>
                    <img src="<?= ASSETS_FULL_URL . 'images/emojis/' . str_replace('feedback_emoji_', '', $row->type) . '.svg' ?>" class="altumcode-emoji-feedback-emoji" data-type="angry" /> <?= language()->notification->emoji_feedback->{$row->type} ?>
                </td>
                <td><?= nr($row->total) ?></td>
            </tr>
        <?php endwhile ?>
...
<script>
    ...
    new Chart(clicks_chart, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: <?= json_encode(language()->notification->emoji_feedback->feedback_emoji_angry) ?>,
                    data: <?= $data->logs_chart['feedback_emoji_angry'] ?? '[]' ?>,
                    borderColor: '#ED4956',
                    fill: false
                },
                {
                    label: <?= json_encode(language()->notification->emoji_feedback->feedback_emoji_sad) ?>,
                    data: <?= $data->logs_chart['feedback_emoji_sad'] ?? '[]' ?>,
                    borderColor: '#ed804c',
                    fill: false
                },
                {
                    label: <?= json_encode(language()->notification->emoji_feedback->feedback_emoji_neutral) ?>,
                    data: <?= $data->logs_chart['feedback_emoji_neutral'] ?? '[]' ?>,
                    borderColor: '#8f8f8f',
                    fill: false
                },
                {
                    label: <?= json_encode(language()->notification->emoji_feedback->feedback_emoji_happy) ?>,
                    data: <?= $data->logs_chart['feedback_emoji_happy'] ?? '[]' ?>,
                    borderColor: '#6c94ed',
                    fill: false
                },
                {
                    label: <?= json_encode(language()->notification->emoji_feedback->feedback_emoji_excited) ?>,
