            ...
            <div>
                <?php if($this->user->plan_settings->campaigns_limit != -1 && $data->campaigns_total >= $this->user->plan_settings->campaigns_limit): ?>
                    <button type="button" data-toggle="tooltip" title="<?= language()->campaign->error_message->campaigns_limit ?>" class="btn btn-primary rounded-pill disabled">
                        <i class="fa fa-plus-circle"></i> <?= language()->dashboard->campaigns->create ?>
                    </button>
                <?php else: ?>
                    <button type="button" data-toggle="modal" data-target="#create_campaign" class="btn btn-primary rounded-pill"><i class="fa fa-plus-circle"></i> <?= language()->dashboard->campaigns->create ?></button>
                <?php endif ?>
            </div>
            ...
            <table class="table table-custom">
                ...
                <?php foreach($data->campaigns as $row): ?>
                    ...
                    <tr>
                        ...
                        <td class="d-none d-md-table-cell"><span class="text-muted" data-toggle="tooltip" title="<?= \Altum\Date::get($row->datetime) ?>"><?= \Altum\Date::get($row->datetime, 2) ?></span></td>
                        <td>
                            <div class="d-flex">
