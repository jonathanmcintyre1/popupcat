...
class AdminQrCodes extends Controller {
    ...
    public function index() {
        ...
        /* Export handler */
        process_export_csv($qr_codes, 'include', ['qr_code_id', 'user_id', 'project_id', 'type', 'name', 'last_datetime', 'datetime'], sprintf(language()->admin_qr_codes->title));
        process_export_json($qr_codes, 'include', ['qr_code_id', 'user_id', 'project_id', 'type', 'name', 'settings','last_datetime', 'datetime'], sprintf(language()->admin_qr_codes->title));
        ...
        $view = new \Altum\Views\View('admin/qr-codes/index', (array) $this);
    ...
    public function delete() {
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            /* Set a nice success message */
            Alerts::add_success(sprintf(language()->global->success_message->delete1, '<strong>' . $qr_code->name . '</strong>'));
