    ...
    public function index() {
        ...
        switch($_POST['type']) {
            ...
            case 'vcard':
                ...
                $vcard->addName($_POST['vcard_first_name'], $_POST['vcard_last_name']);
            ...
            if($qr_code_logo) {
                ...
                if(!\Altum\Plugin::is_active('offload') || (\Altum\Plugin::is_active('offload') && !settings()->offload->uploads_url)) {
                    if(!is_writable(UPLOADS_PATH . 'qr_code_logo' . '/')) {
                        Response::json(sprintf(language()->global->error_message->directory_not_writable, UPLOADS_PATH . 'qr_code_logo' . '/'), 'error');
                    }
                }

                if($_FILES['qr_code_logo']['size'] > $qr_code_settings['qr_code_logo_size_limit'] * 1000000) {
                    Response::json(sprintf(language()->global->error_message->file_size_limit, $qr_code_settings['qr_code_logo_size_limit']), 'error');
                }
            }
