...
class LinkAjax extends Controller {
    ...
    private function create_link() {
        $_POST['location_url'] = mb_substr(trim(Database::clean_string($_POST['location_url'])), 0, 512);
        ...
        if(empty($_POST['domain_id']) && !settings()->links->main_domain_is_enabled && !\Altum\Middlewares\Authentication::is_admin()) {
            Response::json(language()->create_link_modal->error_message->main_domain_is_disabled, 'error');
        }
    ...
    private function create_biolink() {
        ...
        if(empty($_POST['domain_id']) && !settings()->links->main_domain_is_enabled && !\Altum\Middlewares\Authentication::is_admin()) {
            Response::json(language()->create_biolink_modal->error_message->main_domain_is_disabled, 'error');
        }
